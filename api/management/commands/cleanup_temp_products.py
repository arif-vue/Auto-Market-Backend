...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if product_count == 0:
                self.stdout.write(
                    self.style.SUCCESS(f'✅ No expired temporary products found (older than {minutes} minutes)')
                )
                return
            ...
            for product in expired_products:
                self.stdout.write(f'  ⏳ Processing: {product.title} (ID: {product.id})')
                ...
                for img in product.images.all():
                    if img.image and os.path.isfile(img.image.path):
                        try:
                            ...
                            deleted_images += 1
                            self.stdout.write(f'    ✅ Deleted image: {img.image.name}')
                        except Exception as e:
                            image_errors += 1
                            self.stdout.write(
                                self.style.ERROR(f'    ❌ Error deleting image {img.image.name}: {str(e)}')
                            )
                
                # Delete the temp product (this also deletes TempProductImage records via CASCADE)
                product.delete()
                self.stdout.write(f'    ✅ Deleted product: {product.title}')
            ...
            error_msg = f'❌ Error during temp product cleanup: {str(e)}'
